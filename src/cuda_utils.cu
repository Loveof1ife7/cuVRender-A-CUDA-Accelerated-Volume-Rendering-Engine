...
{
    return (unsigned char)(pow(MathUtils::Common::clamp(x, 0.0, 1.0), 1 / 2.2) * 255);
}
...
__host__ __device__ float MathUtils::Common::clamp(float x, float lo, float hi)
{
    return x < lo ? lo : x > hi ? hi
                                : x;
}

__host__ __device__ float MathUtils::Common::Gaussian(float mu, float sigma, float x)
{
